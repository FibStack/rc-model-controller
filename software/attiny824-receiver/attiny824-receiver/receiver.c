...
uint8_t steering = DEFAULT_STEERING;
uint8_t outputs = 0;


void USART0_init()
{
	PORTB.DIR &= ~PIN3_bm; // Set pin 6 as input	USART0.BAUD = (uint16_t)USART0_BAUD_RATE(4800);	USART0.CTRLB |= USART_RXEN_bm; // Enable receiving
}
...
void TCA0_init()
{
	...
	TCA0.SPLIT.LCMP0 = DEFAULT_THROTTLE;
